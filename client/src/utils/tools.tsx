...
import { constants, history } from "./global"
import request, { UxRequest, UxResponse, UxResponseError, toIdentify } from "./request"
...
async function requestHandler<T extends UxResponse | Blob | ArrayBuffer>(params: UxRequest) {
    try {
...
export namespace ux {
     ...
     */
    export function debounce(fn: (...args: any[]) => any, threshold = 1000, mode: DebounceType = DebounceType.POST): (...args: any[]) => ReturnType<typeof fn> {
        ...
        return ((...args: any[]) => {
            const register = () => {
                timer = setTimeout(() => {
                    timer = null
                    // @ts-ignore
                    if (mode & 0b10) return fn.apply(this, args)
                }, threshold)
            }
            ...
            register()
            // @ts-ignore
            if (mode & 0b01) return fn.apply(this, args)
            // @ts-ignore
        }).bind(this)
    }
     ...
     * @param options 可选参数
     */
    export function get(url: string, params?: Record<string, string | number>, options: Pick<UxRequest, 'onDownloadProgress' | 'onUploadProgress' | 'responseType'> = {}) {
     ...
     * @param options 可选参数
     */
    export function post(url: string, data?: any, options: Pick<UxRequest, 'onDownloadProgress' | 'onUploadProgress' | 'responseType'> = {}) {
        return requestHandler<any>({
            ...options,
            url,
            data,
     ...
     * @param variant 类型
     */
    export function notify(message: string, variant?: 'default' | 'error' | 'success' | 'warning' | 'info') {
        enqueueSnackbar({
            message,
    ...
    export function navigateTo(path: string, state?: any) {
        history.push(`${constants.APP_BASE.slice(0, -1)}/${path}`, state)
    }
    export function navigateBack() {
        history.back()
    }
    export function switchTo(path: string, state?: any) {
        history.replace(`${constants.APP_BASE.slice(0, -1)}/${path}`, state)
    }
    ...
    export function getCurrentTheme(): Theme {
        return document.querySelector('html')!.getAttribute('data-theme') ?? 'light' as any
    }
}