 ...
 */
export function useApi<T = void, K = null>(api: ApiMethod<T, K>, options?: {
    ...
    return [
        ...
        async (param, ...args) => {
            ...
            if (options?.showLoading !== false) loader = ux.loading()
            try {
                const res = await api(param, ...args)
                setResult(res)
                loader?.()
                if (options?.successMsg) ux.notify(options.successMsg, 'success')
                return res
            } catch (err: any) {
                loader?.()
                // throw err
