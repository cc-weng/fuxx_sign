...
export type FormRules<T = Record<string, any>> = Record<keyof T, FormRule | FormRule[]>
...
function FormRaw(props: FormProps, ref: Ref<any>) {
    const controller = useRef(new FormStore<FormControlProtocol>({
    ...
    function validate() {
        ...
        Object.entries(props.rules || {}).forEach(([key, rule]) => {
            const value = current.state.value[key] == undefined ? current.state.value[key] : String(current.state.value[key])
    ...
    return <FormContext.Provider value={controller.current}>
        {props.children}
